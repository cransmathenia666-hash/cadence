...
**已知不一致（P2 的表单报错要用，**未决定**）**：错误响应体的 `detail` 有**两种形状**。我们自己 `raise HTTPException` 的 `400` / `409` 是**字符串**（中文，如 `{"detail": "这个计划里已经有一条还没收尾的同名检查点「x」（id=2），不再重复创建"}`）；FastAPI 默认的 `422` 是**数组**，元素形如 `{"type": "date_from_datetime_parsing", "loc": ["body", "due_date"], "msg": "Input should be a valid date or datetime, month value is outside expected range of 1-12", "input": "2026-14-15", "ctx": {...}}`。其中 `loc` 能定位到**具体字段**（前端可据此高亮出错的那个输入框），但 `msg` 是**英文**。前端若直接 `alert(detail)`，遇 `422` 会显示成 `[object Object]`。三个选项：**A** 前端写分支（后端零改动，前端多几行）；**B** 加 `RequestValidationError` 处理器，把 `422` 也归一成中文字符串（丢掉 `loc`，前端无法按字段高亮）；**C** 归一成 `{"detail": "中文说明", "errors": [...]}`——人对人、程序对程序，**推荐**。属改契约，动手前问用户。
