...
3. **不接受 SQL、路径或任意接口地址**：四个工具各自只会碰一张/几张约定的表，参数只允许
   登记过的几个键。
4. **每个工具有自己的字符上限**，超出按下文各自的规矩裁剪（多半是从最旧的截断）。

`TOOLS` 是**唯一入口**：写进 prompt 的工具目录与真正的执行都由它生成，所以「目录里写了、
其实没有」或者「有工具但没告诉它」这两类不一致从结构上不会发生。将来要加「检索经历记忆」
这类工具，就是往 `TOOLS` 里再加一条——本阶段刻意不提前实现（剥离记忆系统那一版的范围）。
"""
...
from . import advisor, blueprint as blueprint_mod, plan
...
def _profile(conn: sqlite3.Connection, plan_id: int, args: dict[str, Any]) -> ToolOutcome:
    """当前有效的长期档案；给了 `category` 就只看那一类。"""
    wanted = args.get("category")
    if wanted is not None and wanted not in advisor.PROFILE_CATEGORIES:
        raise ToolError(
            f"category「{wanted}」不是约定的类别；只能给这五个之一："
...
TOOLS: dict[str, Tool] = {
    ...
    for tool in (
        ...
        Tool(
            ...
            args='可选 {"category": "当前状态"}——只看某一类时给；不给就是全部。',
            arg_names=("category",),
            run=_profile,
        ),
        Tool(
