...
> 仓库根目录：`D:\cadence`
> 主工作树：`D:\cadence`｜**当前在分支 `feat/frontend-a-shell`**（用户 2026-09-19 要求前端走分支、便于重做，`master` 停在 `e866d26`）｜基线：后端 `fe4a5b8`、前端脚手架 `d8f9e3c`；近期：T31 前端 `7d61b63`、P3.6 `d5f3793`｜无远端｜**未提交改动**：P3.7 全部（前后端 + SPEC/todo）＋ 上游会话留下的候选页「落点可就地新建计划」；`.zcode/` 等未入库
> 其他工作树：无
