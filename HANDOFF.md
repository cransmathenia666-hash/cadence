...
> 最近更新：2026-09-15 11:05（本地时区）
> 仓库根目录：`D:\cadence`
> 主工作树：`D:\cadence`｜`master`｜`7eee000`｜无远端｜有未提交改动（见第 3 节）
> 代码基线：`7eee000`（P0 代码 `8d995bd` 未变；`main.py` 已在 P1 重写）
> 其他工作树：无
> 当前唯一目标：P1 后端闭环逻辑——**代码已实现完毕，等用户亲手验收**（项目约定第 9 条：Agent 自报不算完成）
> 下一条动作：用户执行第 6 节的三条验收命令并回报输出；确认后再决定是否进 P2
...
| 真实使用验收（`docs/SPEC.md` 第 9 节） | 未验证 | 成功标准 1 与 5 的**后端部分**已由 E-04 覆盖；2、3、4 及前端部分未做 | 需 P2–P4 完成后 |
...
3. 先判断既有证据是否仍在有效范围：E-01 在 `ledger.py`/`schema.sql`/`test_ledger.py` 未变时可沿用；E-03/E-04 在 `plan.py`/`main.py` 未变时可沿用；E-02 已失效，不要引用。
4. 用户尚未亲手验收 P1 前，不要声称 P1 完成，也不要直接开工 P2。
...
7. 不因工作区缺少外部附件而推断附件从未提供，不把历史阻塞写回当前阻塞。
8. 不删除分支、工作树或用户数据，除非用户明确授权；`data/cadence.db` 里有用户 2 条真实档案数据，不得清库。
