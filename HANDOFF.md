...
> 最近更新：2026-09-18（**T25「找」加宽与 T26 对话式规划都完工**——274 passed、冒烟 10 步；真实库已跑 `db.init` 追上 `plan_chat` 表）
> 仓库根目录：`D:\cadence`
...
| provider 管理页（T11） | 通过 | E-21/E-22/E-24/E-28＋用户手工走查 2026-09-15 | `providers/page.tsx`、那 5 个 provider 函数与 `Provider*` 类型变更后失效；**样式未做** |
...
| 对话式规划与蓝图（T26） | 通过 | E-41 | `backend/app/blueprint.py`、`proposals.decide` 的蓝图分支与 `ProposalDecideIn.selected`、`plan_chat` 表、`/candidates` 对话区与 `/proposals` 的蓝图渲染变更后失效；**真实模型未跑，走查归用户** |
...
**已完成**：P1–P3.5 全部落地（T4–T26）——三级结构、多计划、候选与提案裁定、四问判断、档案三条写入口、T25「找」加宽、T26 对话式规划与蓝图。逐题实现记录在 `tasks/todo.md`。
...
- **口径与约定**：① 候选与提案别混——前者在 `/candidates` 裁定、后者在 `/proposals` 按 `kind` 分流（`plan_blueprint` 是树 + 勾选框）；② `profile_item.category` 只收 `life_habit` / `life_log` / `current_state` / `short_term_goal` / `long_axis` 五个约定令牌（库内自由文本，表外 422）；取代/作废必填理由；同类别同文本回 409 并指明已有 id；③ 四问输出没给 `profile_item_id` id 又不说「依据不足」→ 一律判不合格；④ 档案 #4/#5 是修复前留下的重复，**未清**——想让档案干净去 `/profile` 作废一条；⑤ **`/report` 只列「最新 active 计划」的节点**（T24 按决策未动它）——多计划下想给别的计划报报告，先从该计划页面进去；⑥ 规划对话与蓝图都要「候选已采纳」当前提，不勾 = 整份采纳、一个都没勾回 400。
...
| E-26 / E-27 / 2026-09-16 | T12 四问链路：`pytest -q`（160 passed：147 + 13 条 advisor 单测）与 `smoke_p1.py` 9 步（动了契约与台账写入，按纪律加跑）；随后**真实模型端到端跑通一次**（用户操作，Agent 只读库核对）：提「我要不要学python？」→ 记账 `judge` 成功、549 进 / 598 出 token、5.5 秒，落 `proposal #3`（`material_judgment`）pending | `backend/tests/test_advisor.py` 可复跑；用户库 `llm_call` 与 `proposal #3` 可复查 | 通过：合格输出落 pending 提案且引用 id 真实存在；**没给 id 又不说「依据不足」判不合格**；不合格带原因重试一次、两次不合格抛错且不落提案；无档案不调模型。真实那次质量符合设计，**真密钥体检同时得证** | `advisor.py`、那两条路由、`RequestIn`、`post_json` 请求头、provider id=3 配置任一变更后失效。**真实输出质量只跑过 1 次** |
...
.\.venv\Scripts\python.exe -m pytest -q           # 预期 274 passed
...
npm run dev        # 开发服务，默认 http://localhost:3000（Turbopack，Ready 约 0.3 秒）
                   # 现有页面：/ 计划表（含打勾/跳过/提交交付物） · /new 建计划与建阶段任务 · /report 提交报告 · /candidates 候选清单与规划对话 · /proposals 待裁定提案（含判资料、蓝图勾选） · /providers LLM 提供商 · /profile 长期档案
...
1. 完整读取本文件并核对 Git、工作树与未提交改动；只读取当前目标（**P4：T15–T17**）的源码与规则——需求与验收在 `tasks/todo.md` 的 P4 一节，SPEC 决策见第 18 节；**SMTP 授权码不是开工前提**（T15 可先做接口与空实现）。
