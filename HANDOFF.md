...
**待用户走查（T13，进行中）**：第一次真实「找」已跑通（E-33），否决过两条候选；**剩下要验的是「否决过的不再出现」**——重启后端（`llm.py` 改了超时）后回到 `/ask` 再问一次同类问题，看那两条是否还在。这是成功标准 2 的后半句。同时注意：真模型会不会顽固地再推禁区，单测只验过假上游（E-31），真出问题它会判不合格重试、两次仍命中就直接报错（不落一条候选）——把报错原文发我即可。
