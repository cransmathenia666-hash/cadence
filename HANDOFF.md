...
> 最近更新：2026-09-20（**P3.6 Agent 核心完工**——354 passed、冒烟 **11 步**、lint/tsc exit=0；计划对话改成**受控工具循环**：资料由它自己读、每轮最多 3 次模型调用 + 6 次只读工具、每轮落一行 `agent_run`、计划页多一块「本轮依据」；按用户同日定的策略**剥离记忆系统**，`profile_evidence` 那一整段不做。此前：T31 全完工 + 走查后放宽「一次一件」）
> 仓库根目录：`D:\cadence`
> 主工作树：`D:\cadence`｜**当前在分支 `feat/frontend-a-shell`**（用户 2026-09-19 要求前端走分支、便于重做，`master` 停在 `e866d26`）｜基线：后端 `fe4a5b8`、前端脚手架 `d8f9e3c`；近期：T31 后端 `73162fa`、前端 A 档 `0ddd6fb`、T31 前端 `7d61b63`｜无远端｜**未提交改动**：T31「一次一件」放宽那一批 + 本轮 P3.6（后端三 + 新模块二 + 新表、前端三、SPEC/待办/测试）
> 其他工作树：无
> 当前唯一目标：**P4 触达与导出（T15–T17）**——规格在 `tasks/todo.md` 的 P4 一节；T15 的邮件实现需要用户给 SMTP 授权码，可先做 `notify` 接口与空实现
> 下一条动作：① **用户走查 P3.6**（五步里的四步：问「下一步先做什么」看它自读计划、追问「结合最近情况」看它按需读报告、要求改截止日确认前不变、触发上限看页面说明缺口）；② 待他定：真库里那两组近似重复的任务要不要用「跳过 + 理由」收尾（要写真库，须他点头）；③ 报一个 P4 的短计划给用户点头
...
| 计划对话的「一条可执行建议」（T31 全完工；2026-09-19 走查后放宽成一次可带 1–5 件任务） | 通过 | E-46 | `backend/app/plan_change.py`、`dialogue.say` 的信封与提示词、`_land_suggestion`、提示词里的节点编号、`proposals.decide` 的 `plan_change` 分支、`plan-dialogue.tsx` 确认条与 `/proposals` 渲染变更后失效；**真实模型已跑过（他走查时用的就是它），批量那版走查归用户** |
...
**已完成**：P1–P3.6 全部落地（T4–T33 前后端全完工，含前端 A 档样式与 agent 工具循环）——逐题范围见 `tasks/todo.md`。

**下一步**：P4 的 T15 `notify` 接口与邮件实现、T16 周检查点 job、T17 Markdown 导出。

## 3. 当前开放问题

无阻塞。**待用户亲手做的事**：见「下一条动作」①（P3.6 那四步），外加有空时独立复核 `pytest -q` 与 `tools\smoke_p1.py`（预期值见第 6 节注释）。已回报：P2 四页、`/providers`、真实 provider 与模型（E-27）、T13 清单与去重、采纳自动落阶段、T28/T31 那段对话走查。

候选队列（不影响当前目标）：

- **一个仍有的事**：`start_reason` 未落库（加列 = Ask first），重看旧候选看不到依据，要依据得重问一轮。
- **真实库现状**：计划类数据曾于 2026-09-17 按用户指示**物理清库**（`tools/wipe_plan_data.py`，备份 `.bak-20260917-234923` 可回退），此后用户走查又建了真实计划。**别拿清库时那批旧数字对账**；2026-09-18 跑过 `db.init` 追上两张对话表。T31 的 `plan_change` 提案已由他走查落过真库（含两组近似重复任务）；P3.6 的 `agent_run` 表也已 `db.init` 补上，但**计划对话这条链真库还没跑过**。
...
- **样式与零散口径**：前端共用外壳（`globals.css` + `app-header.tsx`）已落地，8 页统一顶栏。零散口径：`POST /api/report` 无防重复（正解是前端禁用）；框架生成的 `404`/`405` 文案仍是英文；provider 的「地址/模型」清不成空（留空 = 不改）；`ledger.fetch_active` 取的是「初始业务状态」（名字误导、行为没错）；前端 effect 里同步 setState 会被 `react-hooks/set-state-in-effect` 拦（取数写成 `.then` 回调）。**远景**：SPEC 第 17 节第 3 条「档案自动提炼」——T28 已给出它的第一个生产者。
...
- 已定决策共 40 条见 `docs/SPEC.md` 第 18 节，除用户明确要求不重新讨论；台账拆列（方案 A）只在 SPEC 第 17 节第 2 条那三个条件满足时才重开。
...
| E-31 / E-32 / 2026-09-16 | T13 候选链路 + 一次超时修复（用户走查踩到恰好 30 秒的读超时）：`pytest -q`（199 → 201 passed）、`smoke_p1.py` 9 步。修法：`CHAT_TIMEOUT_SECONDS = 180`，超时消息自带出路 | `test_candidates.py` / `test_llm.py` 可复跑 | 通过：条数越界判不合格并带原因重试、两次不合格不落一条；`rank` 即顺序、`why` 无依据又不说「依据不足」判不合格、**禁区命中即判不合格**（含空白/大小写变体）；否决缺理由报错、已裁定 409、不存在 404；失败调用仍记账 | `find_candidates` / `_check_find` / `decide_candidate`、`providers/find.py`、那三条路由、`llm.py` 超时常量变更后失效。**只验假上游** |
| E-33 / E-34 / E-36 / 2026-09-17 | **真实模型「找」两次 + 采纳自动落阶段**（用户拍板）：请求 3 落候选 #1–#5（否决 #1/#5），请求 7 落 #6–#10 且**禁区标题一条没出现**，用户采纳 #6；`llm_call #12`：26.7 秒 / 4975 token。`pytest -q` 204 passed、`smoke_p1.py` 9 步 | 用户库 `candidate` / `llm_call`、`test_candidates.py` 可复跑 | 通过：3–5 条带排序由真 provider 跑通、否决过的不再出现；采纳 → 候选 `accepted` 且计划里出现同名阶段，无归属/撞同名未收尾 → 409 且候选保持 `proposed`、不建节点 | 证的是「否决 → 禁区 → 重问不出现」与采纳落点（`_rejected_titles`、prompt 禁区段、`decide_candidate`、verdict 路由、`VerdictResult`）。**未覆盖**：换说法的同一件事是否漏过（归一化只做去空白与小写）；样本仅两轮 |
...
| E-40 / E-41 / 2026-09-18 | **T25「找」加宽 + T26 对话式规划与蓝图**（规格见 SPEC 决策 35 / 36）：反馈流水进 prompt + 追问槽位；新表 `plan_chat`（6 轮）、蓝图（树 = 版本）、`selected` 勾选建树。验证：`pytest -q` **274 → 279 passed**、`smoke_p1.py` 10 步、lint/tsc exit=0 | `test_candidates.py`、`test_blueprint.py` 可复跑 | 通过：流水只取 `search`/只放已裁定、追问不合格即不落库；未采纳不能聊、6 轮封顶、历史从最早截断；蓝图落 `pending`、新版标旧版 `superseded`、只建勾中的、同名阶段复用、三类冲突在建之前拦下 | `_feedback_block` / `_clarify_problem`、`find.py` 的 prompt 段、`blueprint.py`、`proposals.decide` 的蓝图分支、`plan_chat` 表变更后失效。**只验假上游**；**真实模型未跑** |
...
| E-44 / 2026-09-18 | **T29 提案瘦身与页面分家**（用户走查后拍板；规格见决策 28/29/30 的修订）：删两个规则产提案的生产者、`decide` 拒绝老类型、`plan_tree` 的落后提醒两字段；存量 5 条 pending 推进提案标 `rejected`；新页 `/judge`（四问 + 只读历史）；`/proposals` 装三类。验证：`pytest -q` **301 passed**、`smoke_p1.py` 10 步、lint/tsc exit=0 | `test_proposals.py` / `test_progress.py` / `test_plan.py` 可复跑 | 通过：阶段收尾不再产提案（判定仍在）、落后只出提醒、老类型批准被拒 | 失效条件见第 1 节同名行。**复核轮把「蓝图单独一页」改回「与其余待裁定同住一页」**；**走查归用户** |
| E-45 / 2026-09-18 | **T30 节点字段写入口**（规格见决策 38）：`plan.update_node_fields` = 原地 `UPDATE` + 台账一条 `update_fields` 流水（字段级 before/after + 理由），**id 不变**；路由 `POST /api/plan/nodes/{id}/fields`；界面入口在 `plan-tree.tsx`，蓝图复用同名阶段也走它写交付物。验证：`pytest -q` **312 passed**、`smoke_p1.py` **11 步全绿**、lint/tsc exit=0 | `test_task_layer.py` / `test_blueprint.py` 可复跑 | 通过：id 与引用不断、流水留改前改后、挪截止日带动落后量、清日期、缺理由/无改动/非法日期/给任务设交付物一律拒、改标题撞同名被拒 | 失效条件见第 1 节同名行。**走查归用户** |
| E-46 / 2026-09-18 | **T31 计划对话的「一条可执行建议」**（规格：决策 6/37 修订、**新增 39**、第 11 节四条契约行）——**后端**：新模块 `backend/app/plan_change.py`、`dialogue.say` 改**信封输出**、建议落 `kind=plan_change` 提案（`payload.dialogue_id` 配对，**不加列**）、`view` 回带 `suggestion`、上下文补节点编号、`proposals.decide` 增第四类分支。**前端**（2026-09-19）：确认条、`onChanged` 重取、`/proposals` 第四类渲染。**2026-09-19 走查后放宽**：`tasks` 收 1–5 件、加阶段连它下面的任务一起建、回执改 `added.nodes`。验证：`pytest -q` **332 passed**、`smoke_p1.py` **11 步全绿**、lint/tsc exit=0 | `backend/tests/test_dialogue.py` 可复跑 | 通过：建议一次最多一条、坏建议**一条都不落**、批准后 **id 不变**且留流水、加东西按序建节点、超 5 件与批内重名都拒、**忽略什么都没写**、计划离开进行中时批准被拒而提案保持 pending | 失效条件见第 1 节同名行。**假上游为主**：真实模型只被他走查跑过（真库落过 5 条 `plan_change`，含两组近似重复任务） |
| E-47 / 2026-09-20 | **P3.6 Agent 核心（T32/T33）+ 计划对话全链路复跑**（规格：第 10 节第 6 条、第 11 节两条契约行、决策 6 第四次修订 + 37 修订 + **新增 40**）——新模块 `agent_runtime` / `agent_tools`、四个只读工具、受控循环（3 次模型 + 6 次工具）、新表 `agent_run`、回执新增 `run`/`tools_used`/`stop_reason`、计划页「本轮依据」（折叠块）。验证：`pytest -q` **354 passed**（含 T28/T31 那批对话用例）、`smoke_p1.py` **11 步全绿**、lint/tsc exit=0 | `test_agent.py` / `test_dialogue.py` 可复跑 | 通过：目录=注册表、四个工具只读、跨计划参数被拒、非法工具不判死这一轮、6 次工具与 3 次模型两条闸、撞上限如实说读了什么还缺什么且**一条提案都不落**、成功/上限/失败三种都留一行 `agent_run`（失败挂在你那一句上）、上游挂了也留痕、上下文总闸从最旧丢、历史上限从最早截断、提炼的 409 与批准档案变更真写库 | 失效条件见第 1 节 Agent 那行。**只验假上游**：真实模型未跑，走查归用户 |
...
.\.venv\Scripts\python.exe -m pytest -q           # 全绿即可（P3.6 落地时 354）
