...
> 最近更新：2026-09-18（**T29 页面分家 + T30 节点字段写入口都完工**——311 passed、冒烟 **11 步**、lint/tsc exit=0；推进提案整类删除（存量 5 条已标终态）、判资料/蓝图/待裁定各一页、节点字段现在能原地改了。此前同日：T27/T28 与用户走查报的两处已修）
> 仓库根目录：`D:\cadence`
> 主工作树：`D:\cadence`｜`master`｜基线：后端 `fe4a5b8`、前端脚手架 `d8f9e3c`；近期：T23–T28 见 git 历史，T29 `b41431e`/`86d120e`/`32bd235`、T30 `f7cc58f`｜无远端｜**未提交改动**：无（`AGENTS.md` 的教学协议段被删一事未表态；`.dsh-vision-toolkit/`、`.zcode/`、`辅助对话2.md` 非本项目产物）
> 其他工作树：无
> 当前唯一目标：**P4 触达与导出（T15–T17）**——规格在 `tasks/todo.md` 的 P4 一节；T15 的邮件实现需要用户给 SMTP 授权码，可先做 `notify` 接口与空实现
> 下一条动作：① **把「改字段」接上界面**（T30 只做了后端：计划页的按钮、T26 那句「交付物写不进去」的文案要回头改）；② 计划页对话接上「能改」（用户 2026-09-18 定：T30 之后）；③ 报一个 P4 的短计划给用户点头；④ **用户走查六件未回报**：三级界面与多计划、T25 的追问与流水、T26 的对话与蓝图勾选、T27 的暂停/继续/历史计划、T28 的计划表页对话、**T29 的两个新页与 T30 之后无界面**
...
| 提案瘦身与页面分家（T29）＋节点字段写入口（T30） | 通过 | E-44、E-45 | `plan.update_node_fields`、`proposals.decide`、`plan_tree` 的 `behind_reason`/`advice`、`/judge` 与 `/blueprints` 两页、`/proposals` 的收紧、`/api/judgments` 变更后失效；**两个新页与「改字段」都还没有浏览器走查** |
...
- **口径与约定**：① 候选与提案别混——候选在 `/candidates` 裁定、待裁定提案在 `/proposals`（T29 起只剩 `material_judgment` 与 `profile_change` 两类），蓝图在 `/blueprints`、判资料在 `/judge`；② 档案类别只收五个约定令牌（表外 422）、取代/作废必填理由、同类别同文本回 409；③ 四问输出没给 id 又不说「依据不足」→ 一律判不合格；④ 档案 #4/#5 是修复前留下的重复，**未清**；⑤ **`/report` 只列「最新 active 计划」的节点**；⑥ 两种对话别混（`/candidates` 那段绑候选、6 轮、出树；计划页那块绑计划、不限轮数、只说话与提炼档案提案）；⑦ **落后不再产提案**——计划页显示一句提醒 + 三个建议；⑧ **节点字段现在能原地改**（T30：标题/交付物/截止日，理由必填、id 不变），但**界面入口还没做**。
...
| E-44 / 2026-09-18 | **T29 提案瘦身与页面分家**（用户走查后拍板；规格见决策 28/29/30 的修订）：删两个规则产提案的生产者、`decide` 拒绝老类型、`plan_tree` 带 `behind_reason`/`advice`；存量 5 条 pending 推进提案走台账标 `rejected`；新页 `/judge`（四问 + 只读历史，走 `GET /api/judgments`）与 `/blueprints`；`/proposals` 收紧成两类。验证：`pytest -q` **301 passed**、`smoke_p1.py` 10 步、lint/tsc exit=0 | `test_proposals.py` / `test_progress.py` / `test_plan.py` 可复跑 | 通过：阶段收尾不再产提案（判定仍在）、落后只出提醒、老类型批准被拒但可驳回 | 失效条件见第 1 节同名行。**新页的浏览器走查归用户** |
| E-45 / 2026-09-18 | **T30 节点字段写入口**（规格见决策 38）：`plan.update_node_fields` = 原地 `UPDATE` + 台账一条 `update_fields` 流水（字段级 before/after + 理由），**id 不变**；路由 `POST /api/plan/nodes/{id}/fields`。验证：`pytest -q` **311 passed**、`smoke_p1.py` **11 步全绿**（补了改交付物那一步） | `test_task_layer.py` 可复跑 | 通过：id 与引用不断、流水留改前改后、挪截止日带动落后量、传空清日期、缺理由/无改动/非法日期/给任务设交付物一律拒、改标题撞同名被拒 | 失效条件见第 1 节同名行。**前端入口未做** |
...
npm run dev        # 开发服务，默认 http://localhost:3000（Turbopack，Ready 约 0.3 秒）
                   # 现有页面：/ 计划表（打勾/跳过/交交付物/落后提醒 + 计划对话区） · /new · /report · /candidates 候选清单与规划对话 · /judge 判一份资料 · /blueprints 蓝图待批 · /proposals 待裁定提案 · /providers · /profile
