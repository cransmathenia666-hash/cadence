...
> 其他工作树：无
> 当前唯一目标：**P3（决策入口）进行中**——T10（provider 管理 + 调用记账）已完成，剩下 T11–T14。P1、P2 已完成（P2 的浏览器手工走查仍待用户）
> 下一条动作：做 T11（provider 管理页面）
...
| LLM provider 管理与调用记账（T10） | 通过 | 本轮 E-23 | `llm.py`、或 `main.py` 里那 6 条 provider / llm-calls 路由与请求模型变更后失效 |
| 前端 `frontend/`（T7 + T8 + T9 + 建节点页） | 通过 | 本轮 E-18、E-21、E-22（前端只验 lint 与 tsc） | 相关页面的行为或 `lib/api.ts` 的对应函数改动后失效；**样式未做**；浏览器手工走查待用户 |
| SPEC 第 9 节真实使用验收 | 未验证 | 成功标准 1、5 的后端部分已由 E-05、E-19 覆盖；2、3、4 与前端部分未做 | 需 P2–P4 完成后 |

## 2. 当前目标与完成定义

**目标：P3（决策入口）进行中——T10 已完成，剩下 T11–T14。** 完成定义对应 SPEC 第 9 节成功标准 2 与 3：输入「我不知道该学什么」得到 3–5 条带排序的候选；输入「我发现了某个资料」得到能指回长期档案字段的四问判断。两者都由 LLM 产出**提案**，经用户裁定后才落库。

**P3 进度：** T10 已完成——`backend/app/llm.py` + 6 条接口（providers 的增删改查、连通性体检、`llm-calls` 记账）；密钥**只写不读**（进出只给掩码）、每次调用落一行账、**同一操作最多 3 次调用**（第 4 次抛错中止）。**T11 provider 管理页面、T12 四问判断、T13 候选清单、T14 候选与提案的前端** 都还没做。

**本轮不做 / 下一步：** 触达与 Markdown 导出（P4）、台账拆列（方案 A，触发条件见 SPEC 第 17 节第 2 条）不在本轮。接着做 **T11（provider 管理页面）**——它要用 T10 那 6 条接口。**T12/T13 需要真实 provider 密钥**（决策 19：用现有的 commandcode）；T10 与 T11 都用假 provider 打桩，**不需要密钥**。P2 的浏览器手工走查与样式方案仍待用户。

## 3. 当前开放问题

无阻塞。**待用户亲手做的两件事**：① 在浏览器走查 P2 的四个页面（`/` 计划表、`/new` 建计划与建节点、`/report` 提交报告，以及走一遍完整闭环）；② 有空时独立复核一次 `pytest -q`（预期 `147 passed`）与 `tools\smoke_p1.py`（预期 9 步全绿）。
...
3. 证据沿用规则：**E-19**（后端既有路由的 CORS 与错误形状）在 `config.py` 的 CORS 名单、`main.py` 的两个错误处理器、或那几条既有路由的响应形状未变时可沿用——**新增互不相关的路由不影响它**；**E-23**（T10）在 `llm.py` 与那 6 条 provider / llm-calls 路由未变时可沿用；**E-21 / E-22**（前端）按各自行里"按行为写"的条件判断，`lib/api.ts` 里新增互不相关的函数**不**使其失效；E-18 只证明前端环境可跑；E-08 纯只读可反复跑；E-13 记录用户库的一次数据处置，不受代码变更影响；其余（E-01～E-04、E-06、E-07、E-09～E-12、E-14～E-17、E-20）已失效，不要引用。
