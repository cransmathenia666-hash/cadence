...
> 其他工作树：无
> 当前唯一目标：P1 + 四处 P1 补充**已完成**；**P2 的代码已全部写完**（T7 API 层、T8 计划表页面、T9 报告表单，另有建计划 / 建节点页面），**验收与样式未做**。取数架构为**浏览器直连**
> 下一条动作：用户在浏览器手工走查四个页面（`/` 计划表、`/new` 建计划与建节点、`/report` 提交报告）；样式方案尚未决定
...
**本轮不做 / 下一步：** LLM 与 provider 管理（P3）、触达与 Markdown 导出（P4）、台账拆列（方案 A，触发条件见 SPEC 第 17 节第 2 条）都不在本轮。**P2 的代码已全部写完**：`frontend/app/` 下四个页面（计划表 / 建计划与建节点 / 提交报告）加 `lib/api.ts`，取数走浏览器直连（SPEC 第 18 节第 26 条）。**验收与样式都还没做**——按 AGENTS.md 的验证纪律，浏览器手工走查归用户。
...
3. 证据沿用规则：**E-19**（后端契约）在 `main.py` 的路由或错误形状、`config.py` 的 CORS 名单、`plan.py` 的返回字段、`ledger.py` 的写入语义未变时可沿用；**E-21 / E-22**（前端）按各自行里"按行为写"的条件判断，`lib/api.ts` 里新增互不相关的函数**不**使其失效；E-18 只证明前端环境可跑；E-08 纯只读可反复跑；E-13 记录用户库的一次数据处置，不受代码变更影响；其余（E-01～E-04、E-06、E-07、E-09～E-12、E-14～E-17、E-20）已失效，不要引用。
