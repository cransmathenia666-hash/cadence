...
- **档案现状（2026-09-17 核对，取代原先「只有 2 条主线」的说法）**：用户已用 `/profile` 页把五类补齐——`long_axis` ×3（id 1 已 superseded；id 2「新主线：通用工程基础 + 能上线的项目」、id 3「三周内做出第一个能部署上线的 web 小项目」active）、`short_term_goal` ×2（id 4、5）、`current_state`（id 6「每天能稳定投入 2 小时…」）、`life_habit`（id 7「习惯晚上学，23:30 前睡」）、`life_log`（id 9；id 8 已 void，内容是「工作日有正职，周末空」——用户把它从生活**习惯**改成生活**记录**，走的正是「作废 + 重填」，顺带验证了那条不挡重填的口径）。**四问与「找」从此有真判据可用**。
- **库里留着一对修复前的重复**：`profile_item` #4 与 #5 是一字不差的「三周内做出第一个能部署上线的 web 小项目」（`short_term_goal`）——它们正是用户 2026-09-16 报「补入档案可以重复」时留下的行，当时防重复的代码（`af759fa`）还没生效。**没清**（用户数据不擅自动）：想让档案干净，去 `/profile` 页作废其中一条即可，顺便也是「改 / 作废」那条路的手工走查。注意 #3（`long_axis`）与它们文字相同但类别不同，**按设计不算重复**。
- **两个入口的区别不够明显（T14 待办）**：用户走查时曾在 `evaluate`（判断资料）形态下问「我现在要学习什么」（请求 6，跑的是四问链路，`llm_call #11` 成功）——没出错，但白花了一次判断。T14 做正式页面时要把「判一个资料」与「找方向」两个入口分得更清楚。
...
- **工作区已清（2026-09-17，用户指示提交）**：原先挂着的用户资产全部入库——`AGENTS.md`（教学协议与验证纪律，`3b77bef`）、原始设计图的墓碑清理与 `docs/代码串联图.excalidraw`（`4edac49`）、`待办.md`（用户自己的待改清单，`35e798d`；它**不参与本文件的证据体系**，用户说「做待办里的第 N 条」时照那本做即可）。**唯一未跟踪的是 `.dsh-vision-toolkit/`**：里面只有一个粘贴图片的 `tmp/` 目录，非本项目产物，既没提交也没加进 `.gitignore`，等用户定。
...
| E-32 / 2026-09-16 | 用户走查 T13 时踩到读超时：第一次 `find` **成功**（记账 `llm_call #8`：23.2 秒、738 进 / **4350 出** tokens），否决两条后重问，连续两次在**恰好 30 秒**处 `TimeoutError`（`#9`、`#10`）——撞的是 `post_json` 那个给短请求设的 30 秒默认读超时。修法：聊天类调用改用 `CHAT_TIMEOUT_SECONDS = 180`、体检单给 `CONNECTIVITY_TIMEOUT_SECONDS = 20`，超时错误消息补上「等了多久 + 建议重试」。验证：`pytest -q` **201 passed**（+2 条超时回归：一条锁常量与默认值、一条锁错误消息可操作且失败照样记账）；前端 `npm run lint` 与 `npx tsc --noEmit` 均 exit=0。提交 `ce265ac` | 记账 `data/cadence.db` 的 `llm_call #8～#10` 可复查；`backend/tests/test_llm.py` 可复跑 | 通过：超时常量与 `post_json` 默认值一致且 ≥120 秒、体检短于聊天；超时抛 `LlmError` 且消息含「重试」与等待秒数；失败调用仍写一行记账 | `llm.py` 的超时常量、`post_json` 默认值或 `Operation.chat` 的错误包装变更后失效。**注意**：这条修的是「超时太紧」，不证明长清单必然一次成功——真实耗时取决于模型 |
