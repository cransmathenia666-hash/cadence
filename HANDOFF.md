...
> 最近更新：2026-09-15 11:40（本地时区）
> 仓库根目录：`D:\cadence`
> 主工作树：`D:\cadence`｜`master`｜`7eee000`｜无远端｜有未提交改动（见第 3 节）
> 代码基线：`7eee000`（P0 代码 `8d995bd` 未变；`main.py` 已在 P1 重写）
> 其他工作树：无
> 当前唯一目标：P1 后端闭环逻辑——**用户已亲手走通闭环，仅剩 `pytest` 结果待其回报**
> 下一条动作：用户跑第 6 节的 `pytest -q`（预期 `78 passed`），然后选下一步（补防重复提交 / 进 P2）
...
**目标：** P1 后端闭环逻辑可用——「提交报告 → 节点状态推进 → 落后量可见」这条链能跑通。

**完成定义：** 能用 `/docs` 手工建一个计划与两级节点、提交一条报告，看到节点状态变化与落后量，且 `ledger_event` 留下对应流水。

**验收路径（已可执行）：** 起服务 → `POST /api/plan` 建计划 → `POST /api/plan/nodes` 建阶段与检查点 → `POST /api/report` 提交报告 → `GET /api/plan` 看状态与落后量 → 查库确认 `ledger_event` 有流水。

**本轮不做：** 前端页面（P2）、LLM 与 provider 管理（P3）、触达与 Markdown 导出（P4）。

## 3. 当前开放问题

无阻塞问题。**仅剩一件：用户回报 `pytest -q` 结果**（项目约定第 9 条：验收命令由用户亲手跑）。

候选队列（不影响当前目标）：

- **接口无防重复提交**（实测：同一秒连点两次 `POST /api/plan/nodes` 建出两个相同节点；`POST /api/report` 也会重复落报告）。治理方向：后端对"同一 parent 下同名节点"返回 409，或加幂等键；前端在 P2 点完即禁用按钮。是否做、怎么做，**待用户拍板**。
- 本地库 `data/cadence.db` 有用户手工验收留下的演示数据（计划 1/2/3、节点 1–6、报告 1–4、1 条 pending 提案）。**不得清库**。
- 已向用户提议但未获授权：`backend/tools/show_db.py`（只读打印计划/节点/台账）与 `backend/tools/smoke_p1.py`（一条命令跑完闭环）。
...
| E-05 / 2026-09-15 | **用户亲手**在 `/docs` 操作（真 uvicorn + 自己的 `data/cadence.db`）：建计划→建阶段→建 2 个检查点→3 次报告（含 1 次白点）→`GET /api/plan` | 数据留在用户本地库，可随时复查 | 通过：节点按状态机推进、落后量 5 天→0、阶段收尾产出 1 条 pending `stage_advance` 提案、`GET /api/plan` 七项字段与预测逐项吻合 | 适用于 HEAD `7eee000`；`plan.py`、`main.py` 变更后失效 |
...
3. 先判断既有证据是否仍在有效范围：E-01 在 `ledger.py`/`schema.sql`/`test_ledger.py` 未变时可沿用；E-03/E-04/E-05 在 `plan.py`/`main.py` 未变时可沿用；E-02 已失效，不要引用。
