...
> 最近更新：2026-09-15 14:15（本地时区）
> 仓库根目录：`D:\cadence`
> 主工作树：`D:\cadence`｜`master`｜后端代码基线 `057e0d1`、前端脚手架 `d8f9e3c`｜无远端｜未提交改动仅剩用户三件（见第 3 节）
> 其他工作树：无
> 当前唯一目标：P1（后端闭环）+ 四处 P1 补充（T19 防重复提交、T20 台账语义归属、T21 入参校验、错误响应形状统一）**已完成**；**P2 前端环境已就绪**（脚手架 + 依赖 + dev 可跑），业务代码一行未写。下一步待用户指定
> 下一条动作：用户跑 `pytest -q`（预期 `115 passed`）与 `tools\smoke_p1.py`（预期 9 步全绿），并到 `/docs` 用 `due_date = 2026-14-15` 建节点看 `detail` 是否已是中文一句话；P1 验收回报后即可继续 T7 剩下的两件事（后端 CORS + `frontend/lib/api.ts`）

## 1. 当前状态

| 范围 | 状态 | 证据依据 | 有效范围/失效条件 |
|---|---|---|---|
| 后端 P0 地基（schema / ledger） | 通过 | 本轮 E-15 | E-01 已失效（`ledger.py` 被改）；现由 E-15 覆盖 |
| P1 规则层（状态机 / 报告与落后量 / 周检查点判定，T4–T6） | 通过 | 本轮 E-15 | E-06 已失效（`backend/` 代码与测试被改）；现由 E-15 覆盖 |
| 建节点防重复提交（T19） | 通过 | 本轮 E-15、E-16 | E-07 已失效；HTTP 层由 E-16 复现，其余三条下沉为单测 |
| 台账语义归属（T20，方案 B） | 通过 | 本轮 E-15、E-13 | `ledger.py`、`plan.py` 改动后失效 |
| 接口契约（T21 入参校验 + 方案 C 错误形状 + 服务启动） | 通过 | 本轮 E-15、E-17 | `main.py` 或接口契约改动后失效 |
| 端到端闭环（脚本可复跑） | 通过 | 本轮 E-16 | E-09 已失效；现由 E-16 覆盖 |
| 端到端闭环（用户亲手在 `/docs` 走通） | 通过 | 沿用 E-05，本轮由 E-16 端到端复现 | 用户库数据可随时复查 |
| `backend/tools/` 两个自查脚本 | 通过 | 沿用 E-08，本轮重跑结果逐项一致 | 脚本自身或 `plan.py` 变更后失效 |
| 前端 `frontend/`（仅环境） | 通过 | 本轮 E-18 | 脚手架、依赖或配置改动后失效；**业务代码未写**——T7 的 CORS 中间件与 `lib/api.ts` 都还没做 |
| SPEC 第 9 节真实使用验收 | 未验证 | 成功标准 1、5 的后端部分已由 E-05、E-16 覆盖；2、3、4 与前端部分未做 | 需 P2–P4 完成后 |

## 2. 当前目标与完成定义

**目标：P1 及其补充——已完成。** 完成定义是「在 `/docs` 建计划与两级节点 → 提交报告 → 节点状态按状态机推进、落后量可见 → 台账留流水 → 阶段收尾产出推进提案」，已由用户亲手走通（E-05），本轮再由脚本端到端复现（E-16）。
...
**本轮不做：** 前端业务代码（P2 的 T8/T9）、LLM 与 provider 管理（P3）、触达与 Markdown 导出（P4）、台账拆列（方案 A，触发条件见 SPEC 第 17 节第 2 条）。

**下一步：** 按用户指示先不写前端业务代码。继续 T7 只剩两件事：后端加 CORS 中间件（只放行 `http://localhost:3000`）、写 `frontend/lib/api.ts`（先做 `getPlan()`，错误按方案 C 的形状处理）。再往后 T8 计划表页面、T9 报告提交表单。
...
无阻塞。**仅剩用户侧 `pytest` 与冒烟结果未回报**（按项目约定第 9 条，这两条命令由用户亲手跑）：`pytest -q`（预期 `115 passed`）、`tools\smoke_p1.py`（预期 9 步全绿）。另请用户重看一次 `/docs` 的错误文案——上次看到的 `422` 是**改动前的数组形状**，方案 C 落地后应变成中文一句话。

候选队列（不影响当前目标）：

- **T7 只剩两件事没做**：后端 CORS 中间件未加；`frontend/lib/api.ts` 未写。用户已明确「先把环境配置好，不需要开始写前端代码」，所以这两件等他发话。
...
- 已定决策共 24 条见 `docs/SPEC.md` 第 18 节；除用户明确要求，不重新讨论。
...
|---|---|---|---|---|
| E-01～E-04、E-06、E-07、E-09～E-12、E-14 / 2026-09-14～15 | 已失效的旧证据（P0 台账 10 passed、早期接口探活、P1 78 passed 与 103 passed、临时库闭环与 due_date 校验、T19 五种连发、smoke 9 步、用户在 `/docs` 亲手得到的 422） | 命令与脚本仍在仓库内，可复跑 | 均已失效：`ledger.py`、`plan.py`、`main.py` 与接口契约接连改动，触发了它们各自的失效条件 | 不要引用；对应结论现由 E-15～E-17 覆盖（E-14 里用户看到的 `422` 是**改动前的数组形状**，新形状须重看） |
| E-05 / 2026-09-15 | **用户亲手**在 `/docs` 操作（真 uvicorn + 自己的库）：建计划→建阶段→建 2 个检查点→3 次报告（含 1 次白点）→`GET /api/plan` | 数据留在用户本地库，可随时复查 | 通过：节点按状态机推进、落后量 5 天→0、产出 1 条 pending `stage_advance` 提案、`GET /api/plan` 七项字段与预测逐项吻合 | 基线 `7eee000`；本轮由 E-16 端到端复现，结论仍有效 |
...
| E-15 / 2026-09-15 | `cd backend; .\.venv\Scripts\python.exe -m pytest -q`（Python 3.14.7） | 测试文件在仓库内，可复跑 | 通过：**115 passed**（含本轮新增 12 条错误形状测试） | 基线 `057e0d1`；`backend/` 下代码或测试变更后失效 |
| E-16 / 2026-09-15 | `python tools/smoke_p1.py`（自起临时库 + 空闲端口） | 脚本在仓库内，可复跑；临时库跑完自动删（已核实 `data/` 只剩 `cadence.db`） | 通过：9 步全绿，含未收尾同名 `409`、已收尾后同名 `201`、落后量 5→0、阶段收尾产出提案 | 基线 `057e0d1`；`plan.py`、`main.py` 或接口契约变更后失效 |
| E-17 / 2026-09-15 | 临时库起 uvicorn，对**五条错误路径**各发一次真实请求：非法日期 `422`、同名 `409`、计划不存在 `404`、不存在的路由 `404`、方法不对 `405` | 临时脚本与临时库已删 | 通过：五条**形状全部一致**（键集恒为 `{detail, errors}`、`detail` 恒为字符串、`errors` 恒为数组）；`422` 的 `detail` 已是中文一句话，`errors[0].loc = ["body","due_date"]`；框架生成的 `404`/`405` 文案仍为英文 | 基线 `057e0d1`；`main.py` 或接口契约变更后失效 |
...
.\.venv\Scripts\python.exe -m pytest -q           # 预期 115 passed
...
**待用户执行的验收**：先起后端，跑 `pytest -q` 预期 `115 passed`，再到 `/docs` 用 `due_date = 2026-14-15` 建节点——预期 `422`，且 `detail` 是**中文一句话**（形如「参数不合法：到期日不是有效日期（要零填充的 ISO 日期，如 2026-09-30）」），旁边多一个 `errors` 数组（含 `loc`）。这一步是在看**改动后的新形状**：你上次看到的是旧的数组形状。
...
3. 证据沿用规则：E-13、E-15、E-16、E-17 在 `ledger.py` / `plan.py` / `main.py` / `tests/` 未变时可沿用；E-18 只证明前端环境可跑（不代表任何前端功能），脚手架或依赖变了就失效；E-08 纯只读可反复跑；E-05 的结论由 E-16 复现；E-01～E-04、E-06、E-07、E-09～E-12、E-14 已失效，不要引用。
